...
def predict(reference, search, top_k: int = 16, min_distance: int = 12, score_margin: float = 0.015, profile: str = "full"):
    template = calibrate_reference(reference, official=False)
    bundle = generate_candidates(template, search, top_k=top_k, min_distance=min_distance, profile=profile)
    scored = []
    pitches = {}
    for y, x in bundle["points"]:
        ...
        candidate, pitch = score_candidate(bundle["template"], bundle["search"], y, x, base, profile=profile)
    ...
    diag = build_diagnostics(selected, scored, margin, ambiguous, pitch, search.shape)
...
def main() -> int:
    ...
    for pair in pairs:
        ...
        prediction, diag = predict(reference, search, top_k=args.top_k, min_distance=args.min_distance, score_margin=args.score_margin)
        prediction["id"] = pair["id"]
        predictions.append({"id": pair["id"], "x": prediction["x"], "y": prediction["y"], "confidence": prediction["confidence"]})
        diagnostics[pair["id"]] = {**diag, "id": pair["id"], "runtime_seconds": time.perf_counter() - started}
